  ...
  <div class="app-container calendar-list-container">
    ...
    <el-table :stripe="true" :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column min-width="150px" align="center" label="销售产品">
        <template slot-scope="scope">
          <el-tag v-for="item in scope.row.support_product" :key="item.id" style="margin-right: 2px">{{ item.product_name }}</el-tag>
        </template>
      </el-table-column>
      ...
      <!--<el-table-column width="160px" align="center" label="时间">-->
        ...
        <!--</template>-->
      <!--</el-table-column>-->
      <el-table-column align="center" label="操作" width="400px" class-name="small-padding">
        <template slot-scope="scope">
...
import { getSupportMemberList, editSupportInfo } from '@/api/support_member'
...
export default {
  ...
  methods: {
    ...
    handleOpenInner5(row) {
      console.log(row)
      if (row.support_product.length > 0) {
        row.support_product.map((item, index, array) => {
          this.innerTemp5.product_ids.push(item.id)
        })
      }
      this.innerTemp5.user_account_id = row.id
