<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-input @keyup.enter.native="handleFilter" style="width: 200px;" class="filter-item" placeholder="客服" v-model="listQuery.nickname">
      </el-input>
    ...
    <el-table :stripe="true" :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column min-width="100px" align="center" label="客服部(组)">
        <template slot-scope="scope">
          <span>
    ...
    <!--粉丝查询-->
    <el-dialog :title="innerTableTitle5" :visible.sync="innerTableVisible5" width="90%" >
      <div class="filter-container">
        <el-date-picker
          class="filter-item"
        ...
        <el-input @keyup.enter.native="getInnerList5" style="width: 150px;" class="filter-item" placeholder="备注" v-model="innerListQuery5.remark">
        </el-input>
        <el-input @keyup.enter.native="getInnerList5" style="width: 150px;" class="filter-item" placeholder="粉丝微信号" v-model="innerListQuery5.fans_weixin_account">
        </el-input>
        <el-select @change='getInnerList5' clearable style="width: 100px" class="filter-item" v-model="innerListQuery5.if_consult" placeholder="是否咨询">
          <el-option v-for="item in ifonsultsOptions" :key="item.key" :label="item.label" :value="item.key">
          </el-option>
        </el-select>
        <el-button class="filter-item" type="primary" v-waves icon="el-icon-search" @click="getInnerList5">搜索</el-button>
      </div>
      <el-table :stripe="true" :key='innerTableKey5' :data="innerList5" v-loading="innerListLoading5" element-loading-text="给我一点时间" border fit highlight-current-row
                style="width: 100%">
        ...
        <el-table-column min-width="80px" align="center" label="粉丝微信">
          <template slot-scope="scope">
            <span class="link-type">{{scope.row.product_weixin_fans.weixin_account}}</span><br>
          </template>
        </el-table-column>
        ...
        <el-table-column min-width="120px" align="center" label="撞粉">
          <template slot-scope="scope">
            <template v-if="scope.row.id !== item.id" v-for="item in scope.row.product_weixin_fans.product_weixin_r_product_weixin_fans">
              <el-tag class="link-type">{{item.product_weixin.info}}</el-tag>{{item.support_user_account.nickname}}<br/>
            </template>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="innerDialogFormVisible1">
      <el-form :rules="innerRules1" ref="innerDataForm1" :model="innerTemp1" label-position="left" label-width="20%" style='width: 80%; margin-left:10%;'>
        ...
        <el-form-item label="粉丝微信号">
          <el-input  @blur="productWeixinFansRegist" :disabled="dialogStatus === 'update'" v-model="innerTemp1.weixin_fans_weixin_account"></el-input>
        </el-form-item>
        <el-form-item label="相同粉丝微信号" v-if="Regist.length > 0">
            <template v-for="item in Regist">
              {{item.product_weixin.weixin_account}}<el-tag style="margin-right: 2px;" class="link-type">{{item.product_weixin.info}}</el-tag>{{item.support_user_account.nickname}}<br/>
            </template>
        </el-form-item>
    ...
    <el-dialog :title="innerTableTitle3" :visible.sync="innerTableVisible3" width="90%" >
      <div class="filter-container">
        <el-date-picker
          :disabled="$store.state.user.roles !== 'administrator' && $store.state.user.roles !== 'supportManager' && $store.state.user.roles !== 'supportDirector'"
...
<script>
  ...
  import { getProductWeixinList, getProductWeixinFansPivotList, updateProductWeixinFans } from '@/api/product_weixin'
  ...
  export default {
    ...
    methods: {
      ...
      getInnerList5() {
        ...
        getProductWeixinFansPivotList(this.innerListQuery5).then(response => {
      ...
      handleUpdateFans(row) {
        ...
        this.innerTemp1.weixin_fans_weixin_account = row.product_weixin_fans.weixin_account
        this.innerDialogFormVisible1 = true
        this.$nextTick(() => {
          this.$refs['innerDataForm1'].clearValidate()
        })
      ...
      productWeixinFansRegist() {
        if (!this.innerTemp1.weixin_fans_weixin_account) {
          return false
        }
        getProductWeixinFansRegist({ fans_weixin_account: this.innerTemp1.weixin_fans_weixin_account }).then(res => {
          if (res.data !== []) {
