<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-select v-if="($store.state.user.roles === 'administrator') || ($store.state.user.roles === 'supportDirector')" @change='handleGroupFilter' style="width: 120px" class="filter-item" v-model="listQuery.support_user_account_group_id" placeholder="客服部门">
      ...
      <el-switch
        @change="handleFilter"
        v-model="listQuery.only_new_fans"
        active-color="#409EFF"
        inactive-color="#ddd"
        active-text="新粉排行"
        inactive-text=""
      ...
      getGroupList() {
        getUserAccountGroupList({ user_account_type_id: 2 }).then(response => {
          if (response.data.data) {
            ...
            // alert(this.listQuery.support_user_account_group_id)
