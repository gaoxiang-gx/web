<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :stripe="true" :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column :key="Math.random()" min-width="100px" align="center" label="头像">
        <template slot-scope="scope">
          <span class="link-type" v-if="scope.row.head_img !== undefined">
...
import { getDailyRanking } from '@/api/support_rating'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getDailyRanking(this.listQuery).then(response => {
