<template>
  <div class="app-container calendar-list-container">
        ...
        <el-table-column min-width="100px" align="center" label="联系电话">
          <template slot-scope="scope">
            <span class="link-type"
                  v-if="($store.state.user.roles === 'administrator') || ($store.state.user.roles === 'supportManager') || ($store.state.user.roles === 'supportDirector') || (user_account_id === temp.support_user_account_id)">{{scope.row.phone}}</span>
    ...
    <el-dialog :visible.sync="tableDialogVisible" width="80%">
      <el-table :stripe="true" :key='tableKey3' :data="list1" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
                style="width: 100%">
