<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :stripe="true" :key='innerTableKey2' :data="innerList2" v-loading="innerListLoading2" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column min-width="80" align="center" label="推广人">
        <template slot-scope="scope">
      ...
      <el-table-column min-width="70" align="center" label="返点">
        <template slot-scope="scope">
      ...
      <el-table-column width="180px" align="center" label="退款时间">
        <template slot-scope="scope">
          <span>{{scope.row.created_at}}</span>
        </template>
      </el-table-column>
      <el-table-column width="100" align="center" label="凭证">
        <template slot-scope="scope">
          <span class="link-type" @click="handleOpenImage(scope.row.proof_img)">
              <img style="height:100%;width:100%;" :src="scope.row.proof_img"/>
          </span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="备注" width="230">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="操作" min-width="300" class-name="small-padding">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="info" @click="handleInnerDelete2(scope.row)">删除</el-button>
          <el-button size="mini" type="success" v-if="($store.state.user.roles === 'promotionManager' || $store.state.user.roles === 'administrator' || $store.state.user.roles === 'promotionDirector') && scope.row.check_status === 0" @click="handleInnerDelete3(scope.row, 1)">通过</el-button>
          <el-button size="mini" type="danger" v-if="($store.state.user.roles === 'promotionManager' || $store.state.user.roles === 'administrator' || $store.state.user.roles === 'promotionDirector') && scope.row.check_status === 0" @click="handleInnerDelete3(scope.row, 2)">不通过</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  import { getPromotionChannelAccountList, createPromotionChannelAccount, updatePromotionChannelAccount, deletePromotionChannelAccount, createPromotionChannel, updatePromotionChannel, getPromotionChannelList, createPromotionChannelAccountConsumption, createPromotionChannelAccountRecharge, getPromotionChannelAccountRechargeList, deletePromotionChannelAccountRefund, getPromotionChannelAccountRefundList } from '@/api/promotion_channel'
  import { getProductList } from '@/api/product'
  import { updatePromotionChannelAccountRefundProof } from '@/api/promotion_channel'
  import { checkPromotionChannelAccountRefund } from '@/api/promotion_statistics'
  ...
  export default {
    ...
    methods: {
      handleUpdateProof(row) {
        this.tempProof.promotion_channel_account_refund_id = row.id
      ...
      updateProof() {
        ...
        this.$refs['innerDataForm4'].validate((valid) => {
          if (valid) {
            updatePromotionChannelAccountRefundProof(this.tempProof).then(() => {
      ...
      getInnerList2() {
        ...
        getPromotionChannelAccountRefundList(this.innerListQuery2).then(response => {
      ...
      handleInnerDelete2(row) {
        this.$confirm('此操作将永久删除该消费信息, 是否继续?', '提示', {
          ...
          const ttempData = {
            promotion_channel_account_refund_id: row.id
          }
          deletePromotionChannelAccountRefund(ttempData).then(response => {
            this.$notify({
              title: '成功',
              message: '删除成功',
              type: 'success',
      ...
      handleInnerDelete3(row, state) {
        this.$confirm('是否继续?', '提示', {
          ...
          checkPromotionChannelAccountRefund(ttempData).then(response => {
            const index = this.innerList2.indexOf(row)
            this.innerList2[index].check_status = state
          })
        }).catch(() => {
          this.$message({
            type: 'info',
