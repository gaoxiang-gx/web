<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :stripe="true" :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <!--<el-table-column min-width="130px" align="center" label="权限角色">-->
        <!--<template slot-scope="scope">-->
          <!--<span>{{scope.row.user_account_role.role_name}}</span>-->
        <!--</template>-->
      <!--</el-table-column>-->
      <!--<el-table-column min-width="130px" align="center" label="所属部门">-->
        <!--<template slot-scope="scope">-->
        ...
        <!--</template>-->
      <!--</el-table-column>-->
      <!--<el-table-column min-width="130px" align="center" label="所属分组">-->
        <!--<template slot-scope="scope">-->
          <!--<span>{{scope.row.user_account_group.group_name}}</span>-->
        <!--</template>-->
      <!--</el-table-column>-->

      <!--<el-table-column min-width="100px" align="center" label="账户类型">-->
        <!--<template slot-scope="scope">-->
          <!--<el-tag type="primary">{{scope.row.user_account_type.type_name}}</el-tag>-->
        <!--</template>-->
      <!--</el-table-column>-->
    ...
    <el-dialog :title="clerkName + ' - 负责部门'" :visible.sync="dialogAuth" width="60%" >
      <el-form ref="innerDataForm1" :model="temp" label-position="left" label-width="150px" style='width: 90%; margin-left:5%;'>
        <template v-for="item in temp.support_department_product">
          <el-form-item :label="item.department_name">
            <el-select v-model="item.product_id"
                       multiple
                       style="width: 100%">
              <el-option v-for="item2 in productarray"
...
<script>
  ...
  import { getCopywriteMemberList, editCopywriteMemberManageDepartmentProduct, getCopywriteMemberManageDepartmentProduct } from '@/api/copywrite'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        getCopywriteMemberList(this.listQuery).then(res => {
      ...
      getUserAccountDepartment() {
        getUserAccountDepartmentGroupTree({ user_account_type: 'support' }).then(res => {
          res.data.forEach(item => {
            this.temp.support_department_product.push({
        ...
        getCopywriteMemberManageDepartmentProduct({ copywrite_user_account_id: row.id }).then(res => {
          res.data.forEach(item => {
            const index = this.temp.support_department_product.findIndex(item2 => {
              return item2.support_department_id === item.support_user_account_department_id
            })
          ...
          if (item.product_id.length > 0) {
            item.product_id.forEach(item2 => {
              parm.push({
                department_name: item.department_name,
                product_id: item2,
                support_department_id: item.support_department_id
              })
            })
          }
        })
