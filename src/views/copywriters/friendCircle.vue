  ...
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-select  @change='handleFilter'
        ...
        <el-option v-for="item in userAccountOptions"
                   :key="item.id"
                   :label="item.nickname"
                   :value="item.id">
        </el-option>
    ...
    <el-tabs v-loading="listLoading" v-else v-model="activeName" type="border-card">
      <el-tab-pane :key="key" v-for="(item, key, index) in list" :name="index + ''" :label="item.product.product_name" >
        <el-collapse v-model="activeName1"  @change="handleChange" >
          <el-collapse-item v-for="(item1,key2) in array2(item.date)" :title="item1.date + ' 共 ( ' + array(item1).length + ' )  未审核 ( ' + is_verifiedCount(array(item1)) + ' )'">
            <el-table :data="array(item1)"
                      ...
                      style="width: 100%"
                      stripe>
              <el-table-column align="center" label="发送时间" width="190">
              ...
              <el-table-column align="center" label="标题">
                <template slot-scope="scope">
                  <span class="link-type" @click="handlPreview(scope.row,item,key2)">{{scope.row.script}}</span>
                </template>
              </el-table-column>
              <el-table-column align="center" label="作者 / 修改 / 审核" width="230">
                <template slot-scope="scope">
                  ...
                  <el-tooltip effect="dark" :content="scope.row.created_at" placement="top">
                    <i style="color: #aaa" v-if="scope.row.created_at !== null" class="el-icon-question"></i>
                  </el-tooltip> /
                  ...
                  <el-tooltip effect="dark" :content="scope.row.updated_at" placement="top">
                    <i style="color: #aaa" v-if="scope.row.operator !== null" class="el-icon-question"></i>
                  </el-tooltip> /
                  ...
                  <el-tooltip effect="dark" :content="scope.row.checked_time" placement="top">
                    <i style="color: #aaa" v-if="scope.row.verifieder !== null" class="el-icon-question"></i>
                  </el-tooltip>
                </template>
              </el-table-column>
              <!--<el-table-column align="center" label="修改">-->
                <!--<template slot-scope="scope">-->
                  <!--<el-tooltip effect="dark" :content="scope.row.friend_circle.updated_at" placement="top">-->
                    <!--<span v-if="scope.row.friend_circle.operator !== null">{{scope.row.friend_circle.operator.nickname}}</span><br/>-->
                  <!--</el-tooltip>-->
                <!--</template>-->
              <!--</el-table-column>-->
              <!--<el-table-column align="center" label="审核人">-->
                <!--<template slot-scope="scope">-->
                  <!--<el-tooltip effect="dark" :content="scope.row.friend_circle.checked_time" placement="top">-->
                    <!--<span v-if="scope.row.friend_circle.verifieder !== null">{{scope.row.friend_circle.verifieder.nickname}}</span><br/>-->
                  <!--</el-tooltip>-->
                <!--</template>-->
              <!--</el-table-column>-->
              ...
              <el-table-column align="center" label="操作" width="350">
                <template slot-scope="scope">
                  ...
                  <el-button type="warning" size="mini" @click="handlPreview(scope.row,item,key2)">预览</el-button>
    ...
    <el-dialog :title="time + '--' + product" id="previewDialog" :visible.sync="dialogFormVisible3" width="480px" style="margin-top: 0px">
      <el-card style="width: 440px;">
        <p style="color: red;margin: 0;font-size: 16px;text-align: center;line-height: 20px">{{tempFriendCircle.script}}</p>
        <div style="width: 400px;height: auto;overflow: hidden;margin: 0 auto">
          <img class="preview_weixin" :src="wechat_img" >
          <div class="preview_content">
            <p style="margin: 0;padding: 0;font-weight: 600;color: #565cace6;font-size: 16px;line-height: 26px;">佰芝堂</p>
            <div class="preview_wangeditor" v-html="wangeditorContent"></div>
            <div v-if="tempFriendCircle.type == 1">
    ...
    methods: {
      ...
      getUserAccountList(query) {
        if (query !== '') {
          ...
          getUserAccountList({ nickname: query }).then(response => {
      ...
      handlPreview(row, product, time) {
        ...
        this.tempFriendCircle.type = fctype

        if (fctype === 1) {
          for (const v in row.type_content.pics) {
            this.previewimage.push(row.type_content.pics[v])
          }
          this.$nextTick(() => {
        ...
        if (fctype === 2) {
          this.previewvideo = row.type_content.video_url
        }
        if (fctype === 3) {
          this.tempFriendCircle.link_title = row.type_content.title
          this.tempFriendCircle.link_content = row.type_content.content_link
          this.tempFriendCircle.link_pics = row.type_content.link_pics
          this.linkimage = row.type_content.link_pics
          this.fileList3.push({ name: 'aaa', url: row.type_content.link_pics })
        }
      ...
      // getSupportDepartment(id) {
      //   getCopywriteMemberRSupportDepartment({ copywrite_user_account_id: id }).then(res => {
      ...
      handleUpdate(row, status) {
        ...
        if (status && status === 'copy') {
          ...
          // this.getSupportDepartment(row.author.id)
        }
        this.temp_id = row.id
        getCopywriteFriendCircleDetail({ friend_circle_id: row.id }).then(res => {
          ...
          if (fctype === 1) {
            for (const v in row.type_content.pics) {
              this.fileList.push({ name: v + '', url: row.type_content.pics[v] })
              this.previewimage.push(row.type_content.pics[v])
              this.tempFriendCircle.pics.push(row.type_content.pics[v])
            }
            this.$nextTick(() => {
          ...
          if (fctype === 2) {
            this.previewvideo = row.type_content.video_url
            this.tempFriendCircle.video_url = row.type_content.video_url
            this.fileList2.push({ name: 'aaa', url: row.type_content.video_url })
          }
          if (fctype === 3) {
            this.tempFriendCircle.link_title = row.type_content.title
            this.tempFriendCircle.link_content = row.type_content.content_link
