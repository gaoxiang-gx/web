<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <!--<el-select clearable @change='getList' style="width: 120px" class="filter-item" v-model="listQuery.online_type" placeholder="排班">-->
    ...
    <el-table header-row-class-name="out_table_head"
          ...
          <el-table :data="getArray(scope.row.product_weixin)" border>
            ...
            <el-table-column prop="info" label="状态">
              <template slot-scope="scopeI">
                <el-tag :type="scopeI.row.status | statusWeixinFilter">{{scopeI.row.status|statusWeixinTranslator}}</el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="promotion_channel.channel_name" label="渠道">
            </el-table-column>
            <el-table-column prop="promotion_member.nickname" label="推广人">
            </el-table-column>
            ...
            <el-table-column v-if="$store.state.user.roles === 'administrator' || $store.state.user.roles === 'promotionManager'|| $store.state.user.roles === 'promotionDirector'" align="center" label="操作" width="320" class-name="small-padding">
              <template slot-scope="scopeI">
                ...
                <el-button v-if="scopeI.row.status===1 || scopeI.row.status===2 || scopeI.row.status===3" size="small" type="gray" @click="handleModifyWeixinStatus(scopeI.row,0)">冻结
                </el-button>
                <el-button v-if="scopeI.row.status===0" size="small" type="success" @click="handleModifyWeixinStatus(scopeI.row,1)">开启
                </el-button>
                <el-button v-if="scopeI.row.status===0" size="small" type="error" @click="handleModifyWeixinStatus(scopeI.row,3)">关闭
                </el-button>
              </template>
            </el-table-column>
          </el-table>
      ...
      <el-table-column min-width="100px" align="center" label="产品">
        <template slot-scope="scope">
          <el-tag class="smalltag" v-for="item in scope.row.support_product" :key="item.id" type="primary">{{item.product_name}}</el-tag>
        </template>
      </el-table-column>
